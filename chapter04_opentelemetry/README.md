...
`https://grafana.vm10.handson.cloudnativedays.jp/explore` に接続し、`system_cpu_time_seconds_total`のメトリクスを確認してみます。
...
デプロイされたJaegerのUIは、`https://jaeger.vm10.handson.cloudnativedays.jp/search`から確認することができます。
...
`https://jaeger.vm10.handson.cloudnativedays.jp/explore` に接続し、Service名に`sample-app-blue`を指定してみると、トレースデータが確認できます。
